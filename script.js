...
var urls = {
  ...
  //map: "https://gist.githubusercontent.com/mbostock/4090846/raw/07e73f3c2d21558489604a0bc434b3a5cf41a867/world-110m.json",
...
var projection = d3.geoOrthographic().scale(450).translate([500, 500]);
...
Promise.all(promises).then(processData).then(applyColours);

function applyColours() {
  $('#airports circle').each( function() {
    var dataASN = $(this).attr('data-asn');
    $(this).attr("style", function() {
      return "fill: " + getRandomColor(dataASN);
    });
  });

  $('#flights path').each( function() {
    var dataASN = $(this).attr('data-asn');
    $(this).attr("style", function() {
      return "stroke: " + getRandomColor(dataASN);
    });
  });
}
...
function drawMap(map) {
  var ccc = topojson.feature(map, map.objects.countries);
  //projection.fitSize([width, height], ccc);

  // remove non-continental countries
  //map.objects.countries.geometries = map.objects.countries.geometries.filter(isContinental);

  // run topojson on remaining countries and adjust projection
...
function drawAirports(airports) {
  ...
  let bubbles = g.airports.selectAll("circle.airport")
    ...
    /*.attr("style", function(d) {
      return "fill: " + getRandomColor(d.iata);
    })*/
    .each(function(d) {
...
function drawPolygons(airports) {
  ...
  g.voronoi.selectAll("path")
    ...
    .on("mouseover", function(d) {
      ...
      var tmp = " - " + airport.city;
      if (tmp.length < 4) {
        tmp = "";
      }

      tooltip.text(airport.iata + " " + airport.name + tmp);
...
function drawFlights(airports, flights) {
  ...
  let links = g.flights.selectAll("path.flight")
    ...
    /*.attr("style", function(d) {
      return "stroke: " + getRandomColor(d.iata);
    })*/
    .each(function(d) {
...
// determines which countries belong to the continental united countries
